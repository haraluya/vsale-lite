...
## ⚠️ 使用者習慣
- 回應跟思考過程都使用繁體中文
- 每次完成任務只要有修改檔案都要commit並附上中文說明，但不要主動push
- 完成任務如果有測試必要，都要給我可以執行的測試環境，先build確保通過，然後一定要重啟dev server，正在運行也要刪除重啟，避免有錯誤
- dev server 預設跑在前景，讓我也可以手動關閉
- 所有任務都要以最小改動為原則，不要做多餘的改動，並且不要走捷徑
