...
async function createDatabaseDump(outputPath: string): Promise<void> {
  ...
  try {
    ...
    // ✅ 使用 --schema 參數僅備份 public 和 auth schema（排除 storage）
    const supabaseCommand = `supabase db dump --linked --data-only --schema public --schema auth -f "${outputPath}"`
  ...
  try {
    ...
    // ✅ 使用 --schema 參數僅備份 public 和 auth schema（排除 storage）
    const pgDumpCommand = `pg_dump -h ${DB_CONFIG.host} -p ${DB_CONFIG.port} -U ${DB_CONFIG.user} -d ${DB_CONFIG.database} -F p --data-only --schema=public --schema=auth --no-owner --no-acl -f "${outputPath}"`
