...
export const HomeBlockConfigSchema = z.discriminatedUnion('block_type', [
  ...
  z.object({
    block_type: z.literal('image_carousel_portrait'),
    config: ImageCarouselConfigSchema,
  }),
  z.object({
...
export const CreateHomeBlockSchema = z.object({
  ...
  block_type: z.enum(['image_carousel', 'image_carousel_portrait', 'product_display', 'text_block'], {
    message: '區塊類型必須為 image_carousel, image_carousel_portrait, product_display, text_block 之一',
  }),
  config: z.any(), // 暫時使用 any，由 refine 驗證
  is_active: z.boolean().default(true),
}).refine((data) => {
  // 根據 block_type 驗證對應的 config
  if (data.block_type === 'image_carousel' || data.block_type === 'image_carousel_portrait') {
...
export const UpdateHomeBlockSchema = z.object({
  ...
  block_type: z.enum(['image_carousel', 'image_carousel_portrait', 'product_display', 'text_block']).optional(),
