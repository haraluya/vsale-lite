...
管理員在後台建立首頁廣告區塊，**先選擇區塊類型（圖片輪播、商品展示、文字區塊）**，進入對應的設定表單後**不可變更區塊類型**，設定完成後可查看、編輯（不含類型變更）、刪除區塊。

**Why this priority**: 區塊管理是首頁內容管理的核心，必須優先實作。類型鎖定設計避免上傳資料後切換類型導致的邏輯混亂與資料遺失問題。
...
2. **Given** 管理員點擊「新增區塊」按鈕，**When** 對話框彈出，**Then** 顯示「選擇區塊類型」對話框，包含三個按鈕：「圖片輪播」、「商品展示」、「文字區塊」，以及「取消」按鈕
3. **Given** 管理員在「選擇區塊類型」對話框點擊「圖片輪播」按鈕，**When** 頁面導航完成，**Then** 進入圖片輪播區塊建立頁面，顯示「區塊名稱」欄位、「上傳圖片」欄位（支援最多 5 張）、「自動播放」開關、「輪播間隔（毫秒）」輸入框、「啟用狀態」開關
4. **Given** 管理員在「選擇區塊類型」對話框點擊「商品展示」按鈕，**When** 頁面導航完成，**Then** 進入商品展示區塊建立頁面，顯示「區塊名稱」欄位、「選擇系列」下拉選單、「選擇標籤」下拉選單、「最大顯示數量」輸入框、「啟用狀態」開關
5. **Given** 管理員在「選擇區塊類型」對話框點擊「文字區塊」按鈕，**When** 頁面導航完成，**Then** 進入文字區塊建立頁面，顯示「區塊名稱」欄位、「文字內容」文字框、「字體大小」下拉選單（12px, 16px, 20px, 24px, 32px, 40px, 48px）、「字體顏色」顏色選擇器、「啟用狀態」開關
6. **Given** 管理員在圖片輪播建立頁面上傳 2 張圖片後，**When** 點擊瀏覽器返回按鈕或「取消」按鈕，**Then** 顯示確認對話框「尚未儲存的變更將遺失，確定要離開嗎？」，管理員可選擇「確定離開」或「繼續編輯」
7. **Given** 管理員建立一個圖片輪播區塊（上傳 3 張圖片，自動播放，間隔 5 秒），**When** 點擊「儲存區塊」按鈕，**Then** 顯示「區塊已建立」成功訊息，跳轉到區塊列表頁面，新區塊出現在列表頂部
8. **Given** 管理員在區塊列表點擊「編輯」按鈕，**When** 頁面載入完成，**Then** 進入編輯頁面，**區塊類型欄位顯示為唯讀（灰色背景）**，無法變更類型，其他欄位可正常編輯
9. **Given** 管理員在區塊列表查看剛建立的圖片輪播區塊，**When** 查看區塊卡片，**Then** 顯示縮圖（第一張圖片）、區塊名稱、區塊類型「圖片輪播」、「編輯」與「刪除」按鈕
...
### FR5 - 後台區塊管理（CRUD）
1. **選擇區塊類型**: 點擊「新增區塊」按鈕時，先彈出對話框讓管理員選擇區塊類型（圖片輪播、商品展示、文字區塊）
2. **建立區塊**: 選擇類型後導航至對應的建立頁面，顯示該類型專屬的表單欄位，**建立過程中不可變更區塊類型**
3. **編輯區塊**: 支援修改所有參數（區塊名稱、配置、啟用狀態），**區塊類型欄位顯示為唯讀（灰色背景）**，無法變更類型
4. **離開確認**: 建立或編輯過程中若表單有變更但尚未儲存，點擊「取消」或瀏覽器返回按鈕時，顯示確認對話框「尚未儲存的變更將遺失，確定要離開嗎？」
5. **刪除區塊**: 刪除時自動清理關聯圖片（調用圖片清理函式），刪除前顯示確認對話框
6. **查詢區塊**: 管理員可查看所有區塊（含停用），客戶僅能查看啟用區塊
7. **啟用/停用**: 支援切換區塊的 is_active 狀態
...
1. **區塊類型變更**: 區塊建立後不可變更類型（如圖片輪播 → 文字區塊），需刪除後重新建立
2. **拖曳排序**: 不實作區塊拖曳排序功能（使用上移/下移按鈕替代）
3. **圖片編輯**: 不提供圖片裁剪、濾鏡等編輯功能（上傳原圖）
4. **動畫效果**: 圖片輪播不實作淡入淡出動畫（使用即時切換）
5. **A/B 測試**: 不實作區塊的 A/B 測試功能
6. **定時發布**: 不實作區塊的定時生效功能（僅支援立即生效/停用）
7. **多語言支援**: 不實作區塊內容的多語言版本
8. **統計分析**: 不實作區塊點擊率、商品曝光數等統計功能
9. **商品無限滾動**: 商品展示區塊不實作無限滾動或分頁載入（一次性載入所有商品）
10. **文字 Markdown**: 文字區塊不支援 Markdown 語法（純文字顯示）
...
2. **為何區塊類型建立後不可變更？**
   - **避免資料遺失**: 圖片輪播區塊上傳圖片後切換為文字區塊，圖片將遺失
   - **避免邏輯混亂**: 商品展示區塊的系列/標籤設定與文字區塊的字體顏色無法相容
   - **簡化實作**: 無需處理複雜的資料遷移邏輯（如 JSONB config 結構轉換）
   - **提升 UX**: 強制先選擇類型，避免使用者在錯誤類型下操作後才發現問題
   - **業界慣例**: 參考 Notion、WordPress Gutenberg，區塊類型建立後均不可變更
...
4. **為何使用 CSS scroll-snap 而非第三方輪播庫？**
   - 原生 CSS 效能最佳，支援觸控滑動
