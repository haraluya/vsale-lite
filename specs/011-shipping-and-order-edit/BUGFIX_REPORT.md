...
## 補充修復：前台訂單詳情缺少費用顯示 (2026-01-06)

### 問題
使用者回報：前台（客戶）訂單詳情頁面缺少運費與自訂費用顯示，與後台不一致

### 根本原因
前台訂單詳情頁面 ([app/(shop)/store/orders/[id]/page.tsx](d:\APP\vsale\app\(shop)\store\orders\[id]\page.tsx)) 僅顯示商品明細與優惠券，未實作運費與自訂費用顯示
...
**附註**: 此 Bug 為 BUG-011-001 修復後發現的深層問題，涉及 PostgreSQL Function 錯誤處理機制與前端顯示邏輯。前台訂單詳情頁面的費用顯示遺漏為原始實作時的疏忽，現已補齊。
