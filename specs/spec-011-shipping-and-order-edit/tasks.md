...
- [ ] T001 確認本地 Supabase 正常運行（執行 `supabase start`）
- [ ] T002 檢查現有訂單資料（確認是否有 `confirmed` 狀態訂單）
- [ ] T003 備份生產環境資料庫（執行 `pg_dump` 或使用備份腳本）
- [ ] T004 建立 Feature Branch: `feature/011-shipping-and-order-edit`
...
- [ ] T005 建立 Migration 檔案 `supabase/migrations/20260106_add_shipping_features.sql`
- [ ] T006 在 Migration 中擴展 `tiers` 表（新增 `shipping_fee`, `free_shipping_threshold` 欄位與 CHECK 約束）
- [ ] T007 在 Migration 中擴展 `orders` 表（新增 `shipping_fee` 欄位與 CHECK 約束）
- [ ] T008 [P] 在 Migration 中建立 `order_custom_fees` 表（含 RLS Policy 與索引）
- [ ] T009 [P] 在 Migration 中建立 PostgreSQL Function `calculate_shipping_fee(p_user_id, p_subtotal)`
- [ ] T010 執行 Migration 1 並驗證資料表結構正確（本地環境）

### TypeScript 型別定義與 Schema

- [ ] T011 [P] 擴展 `types/index.ts`（新增 `Tier.shipping_fee`, `Tier.free_shipping_threshold`, `Order.shipping_fee`, `OrderCustomFee` 型別）
- [ ] T012 [P] 擴展 `lib/validations/tier.schema.ts`（新增 `shipping_fee`, `free_shipping_threshold` 驗證規則）
- [ ] T013 [P] 建立 `lib/validations/order.schema.ts` 擴展（新增 `orderCustomFeeSchema`, `orderModificationsSchema`）
...
- [ ] T014 [P] [US1] 擴展 `lib/actions/tiers.ts` 的 `updateTier()` Server Action（新增 `shipping_fee`, `free_shipping_threshold` 參數與驗證）
- [ ] T015 [US1] 擴展 `components/admin/tiers/tier-form.tsx`（新增運費設定區塊 UI：Checkbox、基本運費輸入、免運門檻輸入）
- [ ] T016 [US1] 更新 `app/(admin)/admin/tiers/page.tsx`（確保新欄位正確顯示與傳遞）
...
- [ ] T017 [P] [US2] 擴展 `lib/actions/orders.ts` 的 `createOrder()` Server Action（新增運費計算邏輯：呼叫 `calculate_shipping_fee()` RPC，更新總金額公式）
...
**Checkpoint**: 客戶結帳時正確顯示運費，訂單建立後 `shipping_fee` 欄位正確儲存
