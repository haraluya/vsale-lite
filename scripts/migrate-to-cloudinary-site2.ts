...
async function migrateImages() {
  console.log('🚀 開始遷移站點 2 圖片到 Cloudinary...\n')
...
async function migrateSeries() {
  ...
  for (let i = 0; i < series.length; i++) {
    ...
    try {
      // 上傳到 Cloudinary（使用 vsale-site2 資料夾）
      const result = await cloudinary.uploader.upload(item.image_url, {
        folder: 'vsale-site2/series',
        public_id: item.id,
        overwrite: true,
      })

      // 更新資料庫（儲存 Cloudinary public_id，不帶 / 開頭）
      const cloudinaryPath = `vsale-site2/series/${item.id}`
...
async function migrateProducts() {
  ...
  for (let i = 0; i < products.length; i++) {
    ...
    try {
      const result = await cloudinary.uploader.upload(item.image_url, {
        folder: 'vsale-site2/products',
        public_id: item.id,
        overwrite: true,
      })

      const cloudinaryPath = `vsale-site2/products/${item.id}`
...
async function migrateAnnouncements() {
  ...
  for (let i = 0; i < announcements.length; i++) {
    ...
    try {
      const result = await cloudinary.uploader.upload(item.image_url, {
        folder: 'vsale-site2/announcements',
        public_id: item.id,
        overwrite: true,
      })

      const cloudinaryPath = `vsale-site2/announcements/${item.id}`
    ...
    for (let i = 0; i < config.images.length; i++) {
      ...
      try {
        const result = await cloudinary.uploader.upload(imageUrl, {
          folder: 'vsale-site2/home-blocks',
          public_id: `${block.id}_${i}`,
          overwrite: true,
        })

        // 更新 JSONB 中的 URL（不帶 / 開頭）
        config.images[i].url = `vsale-site2/home-blocks/${block.id}_${i}`
