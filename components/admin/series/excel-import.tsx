...
import { importSeries } from '@/lib/actions/series'
...
export function ExcelImport({ className = '' }: ExcelImportProps) {
  ...
  const handleImport = async (isDryRun: boolean) => {
    ...
    try {
      ...
      const result = await importSeries(selectedFile, {
        dry_run: isDryRun,
        skip_errors: false,
      })
      ...
      if (result.data.error_count === 0) {
        if (isDryRun) {
          ...
          toast.success(result.message || `成功匯入 ${result.data.success_count} 筆系列資料`)
          handleClearFile()
          // 匯入成功後重新載入頁面
          setTimeout(() => {
  ...
  return (
    <div className={`space-y-4 ${className}`}>
      ...
      <button
        ...
        className="w-full flex items-center justify-between rounded-none border-3 border-black bg-yellow-300 px-6 py-3 font-bold shadow-neo hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all"
      >
        ...
        {!selectedFile ? (
          <label className="block cursor-pointer">
            <input
              ...
              className="hidden"
            />
            <div className="border-3 border-dashed border-gray-300 rounded-none p-8 text-center hover:border-black transition-colors">
          ...
          <div className="flex items-center gap-3 p-4 border-3 border-black bg-gray-50">
        ...
        {selectedFile && (
          <div className="flex gap-3 mt-4">
            <button
              ...
              className="flex-1 px-4 py-2 rounded-none border-3 border-black bg-yellow-300 hover:bg-yellow-400 shadow-neo hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
            >
            ...
            <button
              ...
              disabled={isImporting}
              className="flex-1 px-4 py-2 rounded-none border-3 border-black bg-green-300 hover:bg-green-400 shadow-neo hover:translate-x-[2px] hover:translate-y-[2px] hover:shadow-none transition-all duration-150 disabled:opacity-50 disabled:cursor-not-allowed"
            >
          ...
          {showResults && importResult && (
            <div className="rounded-none border-3 border-black bg-white p-6 shadow-neo">
              ...
              {importResult.dry_run && importResult.error_count === 0 && (
                <div className="mt-4 p-4 border-3 border-green-600 bg-green-50">
