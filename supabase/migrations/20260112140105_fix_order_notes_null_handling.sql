-- ============================================================================
-- Migration: 修復訂單備註 NULL 值處理問題
-- Feature: Bug Fix
-- Date: 2026-01-12
-- Description: 修正 update_order_with_modifications 函數中備註更新的 NULL 處理邏輯
-- ============================================================================

-- 問題描述:
-- 當 p_modifications->'notes'->>'new_notes' 回傳 JSON null 時
-- PostgreSQL 會將其解釋為空字串而不是 SQL NULL
-- 導致備註無法正確清空或更新
--
-- 修復方式:
-- 使用 NULLIF 函數將空字串轉換為 NULL
-- 或使用 CASE WHEN 判斷 JSON null
...
COMMENT ON FUNCTION update_order_with_modifications IS '批次修改訂單（商品、費用、運費、備註）並記錄修改歷程，使用 NULLIF 正確處理備註的空值';
...
BEGIN
  ...
  IF v_function_exists THEN
    ...
    RAISE NOTICE '  - 修正備註更新的 NULL 處理邏輯';
    RAISE NOTICE '  - 使用 NULLIF 將空字串轉換為 NULL';
  ELSE
