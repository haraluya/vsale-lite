-- ============================================================================
-- Migration: 修復組合優惠商品庫存管理
-- Feature: 021-combo-deals
-- Date: 2026-01-30
--
-- 修復內容：
-- 1. mark_order_as_shipping：新增組合優惠商品庫存扣減
-- 2. cancel_order_and_restore_stock：新增組合優惠商品庫存回補
--
-- 業務規則：
-- - 組合優惠商品與普通商品一樣扣減庫存
-- - 商品數量從 order_combo_deal_items.snapshot->>'selectedProducts' 中提取
-- - 支援負庫存（憲章 VI）
-- ============================================================================

-- ============================================================================
-- 1. 修復 mark_order_as_shipping：新增組合優惠商品庫存扣減
-- ============================================================================
...
BEGIN
  ...
  -- 從 order_combo_deal_items 的 snapshot->'selectedProducts' 中提取商品與數量
  FOR v_combo_item IN
    SELECT snapshot FROM order_combo_deal_items WHERE order_id = p_order_id
  LOOP
    -- 遍歷 snapshot.selectedProducts 陣列
    FOR v_selected_product IN
      SELECT * FROM jsonb_array_elements(v_combo_item.snapshot->'selectedProducts')
    LOOP
      -- 扣減庫存
      UPDATE products
...
-- ============================================================================
-- 2. 修復 cancel_order_and_restore_stock：新增組合優惠商品庫存回補
-- ============================================================================
